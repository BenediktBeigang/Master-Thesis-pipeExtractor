...
# --- Finale Ausgabe ---
print(f"\n=== Zusammenfassung: {len(found_pipes)} Rohre gefunden ===")
for i, pipe in enumerate(found_pipes):
    print(f"Rohr {i+1}: Fitness={pipe['fitness']:.2f}")

# Alle gefundenen Rohre in OBJ-Datei schreiben
...
# Gefundene Rohre als CSV speichern
# if found_pipes:
#     pipes_df = pd.DataFrame(found_pipes)
#     pipes_df.to_csv(f"./output/{timestamp}_found_pipes.csv", index=False)
